...
fn fixture_triplet(index: usize) -> (&'static str, &'static str, &'static str) {
    match index % 3 {
        0 => ("BTCUSDT", "trend_long_quality", "committee_bull_breakout"),
        1 => (
            "ETHUSDT",
...
-> Result<()> {
    ...
    let result = async {
        ...
        let mut proposals = Vec::new();
        {
            ...
            for index in 0..100 {
                let (symbol, signal_fixture, proposal_fixture) = fixture_triplet(index);
        ...
        for run_id in &run_ids {
            assert_eq!(runtime.run_artifact_count(*run_id), 2);

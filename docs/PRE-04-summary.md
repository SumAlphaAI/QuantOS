...
| 对照现有 Proto/API client | 达成 | 6 个 Proto 文件、38 messages、15 enums、2 services、7 RPC、48 JSON Schemas；BFF OpenAPI 1.1.0 有 55 operations/41 schemas，generated manifest 精确一致 |
...
1. 将旧台账的 BFF `1.0.0 / 42 operations` 更新为实际 `1.1.0 / 55 operations / 41 schemas`，并纳入 generated manifest 双向比对。
...
2. OpenAPI 1.1.0 的 C17 P15/P17 面仍是 Contract Mocked，staging provider/consumer 签署未执行。

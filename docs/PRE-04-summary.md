...
| 对照现有 Proto/API client | 达成 | 6 个 Proto 文件、38 messages、15 enums、2 services、7 RPC、50 JSON Schemas；BFF OpenAPI 1.3.0 有 62 operations/51 schemas，generated manifest 精确一致 |
...
1. BFF 当前基线为 `1.3.0 / 62 operations / 51 schemas`，并纳入 generated manifest 双向比对。
2. 将旧 Proto `17 messages / 14 enums` 更新为实际 `38 messages / 15 enums / 2 services / 7 RPC / 50 JSON Schemas`。
...
2. OpenAPI 1.3.0 的 C01/C10/C17 P15/P17 已有本地参考 provider，staging provider/consumer 签署仍未执行。

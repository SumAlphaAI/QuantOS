...
> 任务：PRE-02 设计系统预研（FEP-0）  状态：交付待评审
...
2. `check:design` 需在 PRE-03/CI 整合阶段接入 CI job（建议与 PRE-06 测试基线合并）。
3. 领域组件八态 Storybook 全量覆盖随各 `UI-Pxx` 任务逐页补齐，基准为 StateBadge 示例与组件清单第 3 节约定。
4. 本任务交付物需并入 G0 联合评审（同 PRE-01 评审记录流程）；PRE-01 遗留项 3（场景表文案引用切换为 i18n key）现已可执行。

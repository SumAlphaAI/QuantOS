...
- 复验日期：2026-09-21
- 本轮检查基线：`f5579c7`；本报告及测试修正由后续独立提交归档
...
本轮已将容量验收改为循环调用 `poll_outbox_once`，检查 10,000 个唯一事件的处理、副作用、checkpoint 与完整回放集合；恢复从查询调用开始到全部结果反序列化完成的五秒计时，并增加源码 Gate 防止重新引入 SQL 完成状态夹具或 EXPLAIN 替代计时。
...
| C16 | 10,000 条无丢失与最终一致 | PARTIAL | 已改用正式消费者；旧回执使用 SQL 直接写完成状态，新测试待数据库执行回执 |
...
| C19 | correlation 完整链查询 ≤5 秒 | PARTIAL | 已恢复完整客户端取回计时；旧 8.452ms 仅为数据库执行时间，新测量待回执 |
...
| C27 | 覆盖率及 nightly branch Gate | PARTIAL | 现有核心 line/region 与 branch 达标，但排除 pg.rs、supabase_storage.rs，不能证明持久化路径覆盖率 |
...
| 证据 | 结果与范围 |
|---|---|
| 本轮 `QUANTOS_SKIP_ENV=1 make f05-check` | PASS；source Gate、4 项负向测试、22 个库测试；核心 line 95.03%、region 94.29%，不包含持久化适配器 |
...
| 本轮修订后的数据库容量测试 | **NOT RUN / NO RECEIPT**；本轮未执行远端数据库写操作，本机未找到可用 PostgreSQL 服务工具 |
...
|---|---|---|---|---|
| A07 | 中危 | F05 覆盖率 Gate、PostgreSQL / Supabase Storage 适配器 | Makefile 与 nightly 使用 `--lib --ignore-filename-regex '(pg\|supabase_storage)\.rs'`，将原问题关注的持久化路径排除；真实 DB 功能成功不等于这些路径的覆盖率达标 | 持久化错误、重试、事务和恢复分支缺乏可量化回归证据；**OPEN** |
| A09 | 中危 | PostgreSQL 容量、一致性和 correlation 查询验收 | 旧容量夹具用 SQL 制造完成状态，旧五秒断言只计数据库执行；本轮已改为正式消费者和完整客户端查询计时 | 10,000 条真实消费与完整链 ≤5 秒仍需修订测试的新回执；**FIX_VALIDATION** |
...
1. **A07**：对 PostgreSQL 与 Supabase Storage 的真实集成测试采集覆盖率，列出文件范围、排除项及理由；补齐持久化失败、重试与回滚测试。不得以排除 adapter 后的核心覆盖率宣称原问题全部关闭。

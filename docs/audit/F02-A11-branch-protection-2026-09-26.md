...
> 结论：**required checks 阻断/恢复证据缺口 CLOSED / PASS**；F02 整体仍待当前主线可重复构建问题解决。原始证据见 [证据目录](./evidence/F02-A11-2026-09-26/README.md)。

## 一、任务完成概况

本次关闭范围是 F02-A11 的有效 required checks、失败阻断与恢复证据缺口。所有故障注入及恢复合并只面向隔离分支 `codex/f02-a11-protected-target`；配置修复与最终报告位于独立主线收尾分支。

主线基线为 `ac73a95a758cc95385b69e9d6f9376cef4a2af0b`。正式 [QuantOS CI](https://github.com/SumAlphaAI/QuantOS/actions/runs/36240334477) 已通过，正式签名/下载复验、SCA、许可证与隔离数据库证据均绑定此 SHA。但独立 [F01 Clean Room](https://github.com/SumAlphaAI/QuantOS/actions/runs/36240334483) 的 reproducibility 检查失败，当前主线不能表述为 8/8 全绿或 F02 全量 ACCEPTED。
...
### 3.2 仍需处理：当前主线 Terminal 可重复构建失败
...
- 影响：当前主线 `acceptance (reproducibility)` 为 failure，阻止完整主线验收。不能用另一 SHA 的绿色运行覆盖该失败，也不能删除该 required check。
...
1. 已完成恢复 PR 的 8/8 检查、固定 head 隔离合并和服务端规则 pass；今后规则变更时重复相同正反对照。
2. 已恢复规则集仅覆盖 main，并读取确认 strict、8 项 App 来源检查和零 bypass。隔离分支和已合并 PR 仅保留用于审计，不进入 main。
3. 将本次 Playwright 修复、回归和证据作为独立 Git 提交交付；后续合入 main 的新 SHA 必须取得自己的主线回执。
4. 对 ac73a95 的 Terminal 构建差异另行保留三次实际输出，定位 chunk 内容差异并修复后重新收集主线同 SHA 的完整 8 项检查及正式下载验签。此前 F02 整体验收保持待验收。

...
- 来源：第一期总体计划 F01 中的 Desktop workspace，以及原 PRE-03、PRE-05、PRE-06 中的 Desktop 部分。
- 动作：建立或复核 `apps/terminal-desktop`、`packages/platform`、Tauri/Rust pin、桌面 Cargo lock、`frontendDist`、capability/CSP；建立 desktop-local、desktop-integrated、desktop-staging 配置；建立原生测试与候选包证据格式。
...
- 动作：实现通知（包括风险、审批、订单与运维事件）、窗口、文件、缓存、更新、诊断、深链平台 adapter；业务组件不得出现 `isDesktop` 分叉。
...
- 动作：将第一期 Web 的 Research、第三方 Research adapter、策略、风险/审批、订单、审计、对账和运维旅程以同一 fixture 在 Desktop 候选包复跑；增加冷/热深链、系统通知、多窗口/多显示器、布局恢复、离线只读、文件导入、更新/回滚与 200% 缩放场景。
- 自动化目标：在 Makefile/CI 固化 `e2e-desktop`，并与 Desktop 视觉回归、可访问性、原生 E2E 和更新/回滚测试组成独立的第二期测试矩阵。
...
| D2 功能 | UI-604、UI-P16，以及 Research/策略/执行/审计共享业务旅程在本地候选包通过 | 已签名/可分发 |
...
| 原位置/标识 | 第二期归属 |
|---|---|
| 总体计划 1.1/1.2 的 Desktop 交付范围、密钥与共享业务边界 | 本计划第 1–3 节 |
| 总体计划 2.2 的 macOS/Windows 兼容回归 | DESK-QA-001 / D3 |
| 总体计划 F01 的 `apps/terminal-desktop` 与 Desktop workspace | DESK-PRE-01 |
| 总体计划 TP01 质量 Gate 的 Web/Desktop Research E2E | DESK-QA-001 / D2 |
| 总体计划 U01 的 Tauri adapter 与双端 Playwright | UI-604 / DESK-QA-001 |
| 总体计划 S04 的双端策略视觉/功能回归 | DESK-QA-001 / D2 |
| 总体计划 X06 的 Desktop 通知与双端执行/审计/运维回归 | UI-604 / DESK-QA-001 / D2 |
| 总体计划 R1 Gate 的 Web/Desktop Research 一致性 | DESK-QA-001 / D2 |
| 总体计划自动执行命令中的 `e2e-desktop` | DESK-QA-001 / D2–D3 |
| 总体计划禁止项中的 Desktop 旁路、复制业务状态机和离线写入 | 本计划第 1、2、8 节 |

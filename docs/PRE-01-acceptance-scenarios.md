...
## 3. 官网验收场景（WEB-01–WEB-07）

### WEB-01 官网首页

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-01-S1 | 默认 | 品牌主张、提议—审批—执行—留证闭环、三项信任承诺与访问申请 CTA 完整；禁用词扫描通过 |
| ACC-WEB-01-S2 | 加载 | 核心标题与 CTA 优先渲染且无布局跳变；页面 LCP ≤2.5s |
| ACC-WEB-01-S3 | 空 | 可选动态案例为空时保留产品价值与访问申请入口，不出现空白主区域 |
| ACC-WEB-01-S4 | 错误 | 非关键内容加载失败时局部降级，品牌主张、合规声明和 CTA 仍可访问 |
| ACC-WEB-01-S5 | 无权 | 首页不请求或渲染任何 Terminal 业务数据；受保护入口跳转登录 |
| ACC-WEB-01-S6 | 陈旧 | 页脚显示内容版本/更新时间；过期公告醒目标注且不冒充当前能力 |
| ACC-WEB-01-S7 | 离线 | 已渲染品牌与安全边界可阅读；需要网络的 CTA 标明离线且不提交 |

### WEB-02 官网产品页

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-02-S1 | 默认 | Research、Strategy Governance、Risk & Execution、Audit 四项能力及其边界完整呈现 |
| ACC-WEB-02-S2 | 加载 | 能力卡使用稳定骨架并保留标题层级，辅助资源不阻塞正文 |
| ACC-WEB-02-S3 | 空 | 某能力暂无扩展材料时显示能力边界与文档入口，不显示虚构案例 |
| ACC-WEB-02-S4 | 错误 | 能力详情资源失败时局部提示并提供重试/文档入口，其他能力正常 |
| ACC-WEB-02-S5 | 无权 | 产品页仅展示公开能力说明，不泄露账户、策略、订单或内部运行状态 |
| ACC-WEB-02-S6 | 陈旧 | 能力描述绑定内容版本；已下线能力标注历史状态且 CTA 不可用 |
| ACC-WEB-02-S7 | 离线 | 已缓存能力说明可读；外部文档和访问申请动作显示需联网 |

### WEB-03 官网架构与安全页

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-03-S1 | 默认 | Agent/风控边界、事件审计、权限、数据与供应链治理说明完整且无安全夸大 |
| ACC-WEB-03-S2 | 加载 | 架构图提供文字替代；图片加载不阻塞安全边界正文 |
| ACC-WEB-03-S3 | 空 | 图形资源缺失时展示等价文本结构与安全责任边界 |
| ACC-WEB-03-S4 | 错误 | 图形或附件失败时显示可解释降级，不隐藏关键限制与风险声明 |
| ACC-WEB-03-S5 | 无权 | 公开架构不包含内部拓扑、密钥、租户标识、漏洞细节或受限 Runbook |
| ACC-WEB-03-S6 | 陈旧 | 安全声明显示适用版本/日期；过期待更新内容显式标记，下载链接不静默指向旧版 |
| ACC-WEB-03-S7 | 离线 | 安全边界文本可读；需联网的报告下载禁用并说明原因 |

### WEB-04 官网使用场景页

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-04-S1 | 默认 | 研究团队、量化开发、交易与风控协作场景均说明前提、角色边界和非目标 |
| ACC-WEB-04-S2 | 加载 | 场景导航与首个场景骨架稳定，切换时保留当前标题与焦点 |
| ACC-WEB-04-S3 | 空 | 筛选无匹配场景时提供清除条件和联系支持入口 |
| ACC-WEB-04-S4 | 错误 | 场景内容加载失败可重试，不以营销占位替代合规边界 |
| ACC-WEB-04-S5 | 无权 | 公开场景只使用匿名示例，不展示客户、账户、策略或业绩数据 |
| ACC-WEB-04-S6 | 陈旧 | 场景标注适用产品阶段；未交付能力明确标为规划中 |
| ACC-WEB-04-S7 | 离线 | 已加载场景可浏览；访问申请与外链动作禁用或说明需联网 |

### WEB-05 官网文档中心

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-05-S1 | 默认 | 架构、SDK、API、部署、Runbook 与变更日志入口按公开范围展示且链接有效 |
| ACC-WEB-05-S2 | 加载 | 目录和搜索显示可访问骨架，焦点与读屏状态可感知 |
| ACC-WEB-05-S3 | 空 | 无匹配搜索结果时给出清除条件、推荐分类与联系支持入口 |
| ACC-WEB-05-S4 | 错误 | 索引或文档加载失败时保留查询条件，显示重试与 correlation ID（如服务端提供） |
| ACC-WEB-05-S5 | 无权 | 受限文档返回统一登录/申请入口，搜索结果不泄露标题、摘要或对象存在性 |
| ACC-WEB-05-S6 | 陈旧 | 文档显示版本和更新时间；旧版醒目标注并链接当前版本，无失效下载引用 |
| ACC-WEB-05-S7 | 离线 | 已缓存公开文档只读；搜索、下载与受限文档跳转说明需联网 |

### WEB-06 官网访问申请页

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-06-S1 | 默认 | 团队、用途、市场、预期模式、隐私与条款同意字段完整；服务端防滥用启用 |
| ACC-WEB-06-S2 | 加载 | 提交后立即进入 pending 并防重复提交，字段保持只读直至结果返回 |
| ACC-WEB-06-S3 | 空 | 首次进入显示最小必填说明、隐私用途和支持入口，不预填敏感信息 |
| ACC-WEB-06-S4 | 错误 | 4xx/5xx/429 显示可执行原因与等待时间，不丢失非敏感已填内容，不推断账户存在性 |
| ACC-WEB-06-S5 | 无权 | 地域/策略限制由服务端返回说明；前端不提供绕过入口且不泄露内部规则 |
| ACC-WEB-06-S6 | 陈旧 | 条款或隐私版本变化使旧同意失效，要求重新确认后才能提交 |
| ACC-WEB-06-S7 | 离线 | 表单可保留未提交草稿但提交禁用；恢复后不自动发送 |

### WEB-07 官网登录页

| 场景 ID | 状态 | 验收标准 |
|---|---|---|
| ACC-WEB-07-S1 | 默认 | 仅提供受控 SSO/OIDC、MFA 说明、访问申请与支持入口，不采集业务凭据 |
| ACC-WEB-07-S2 | 加载 | OIDC 跳转 pending 防重复；回调期间无可重放的交互提交 |
| ACC-WEB-07-S3 | 空 | 无可用 IdP 配置时仅显示访问申请/支持入口，不回退到不安全登录 |
| ACC-WEB-07-S4 | 错误 | 登录/回调错误不透露账户存在性；URL、日志与页面不含 token，提供安全重试 |
| ACC-WEB-07-S5 | 无权 | 无 Terminal 权限时显示统一受限状态与申请入口，不泄露租户或资源信息 |
| ACC-WEB-07-S6 | 陈旧 | 会话或授权上下文过期时重新认证；return path 仅允许站内安全路径 |
| ACC-WEB-07-S7 | 离线 | 登录与回调不可用并说明需联网；不缓存认证令牌或自动重试提交 |
...
| WEB-01 | WEB-101 | 无（静态） | 无（静态） | F01/F02 | ACC-WEB-01-* | G1 |
| WEB-02 | WEB-101 | 无（静态） | 无（静态） | F01/F02 | ACC-WEB-02-* | G1 |
| WEB-03 | WEB-101 | 无（静态） | 无（静态） | F02/F06/F09 | ACC-WEB-03-* | G1 |
| WEB-04 | WEB-101 | 无（静态） | 无（静态） | F01–F09 | ACC-WEB-04-* | G1 |
| WEB-05 | WEB-101 | 公开文档索引 | BFF-FE-001（受限入口） | F01/F02 | ACC-WEB-05-* | G1 |
| WEB-06 | WEB-101 | Access Request（C01） | BFF-FE-001 | F06 | ACC-WEB-06-* | G1 |
| WEB-07 | WEB-101 | Auth（C01） | BFF-FE-001 | F06/L03 | ACC-WEB-07-* | G1 |
...
- 七态场景：每个页面使用独立 ID 定义 S1–S7 共 7 条，31 × 7 = 217 条，全部定义完毕；官网不再以聚合场景代替逐页验收。

...
> 状态：**56 个 published operation 已由 OpenAPI 1.2.0 生成；其余 operationId 仅为 A1 planned 基线，仍由 A2–A6 交付 schema/path/provider**

## 1. 回填前提与当前状态

- 执行计划 5.7 要求：每个页面开发前建立一行 Page API Coverage 记录；每个 `UI-Pxx` 必须可追踪到 operationId。
- **版本化 [quantos-bff.v1.yaml](../bff/openapi/quantos-bff.v1.yaml) 当前为 1.2.0，56 个 operationId 已发布并生成 client/schema/MSW。**
...
mock 版本说明：生成 TypeScript client、Zod、JSON Schema 与 MSW handlers 与 OpenAPI 同源；当前版本 `1.2.0`。C01/C17 P15 已有本地 Rust 参考 provider，但仍需 staging 消费者/provider 签署后才能升级为 Integrated；P16 仍属 BFF-FE-011。

...
> 状态：**62 个 published operation 已由 OpenAPI 1.3.0 生成；其余 operationId 仅为 A1 planned 基线，仍由 A3–A6 交付 schema/path/provider**

## 1. 回填前提与当前状态

- 执行计划 5.7 要求：每个页面开发前建立一行 Page API Coverage 记录；每个 `UI-Pxx` 必须可追踪到 operationId。
- **版本化 [quantos-bff.v1.yaml](../bff/openapi/quantos-bff.v1.yaml) 当前为 1.3.0，62 个 operationId 已发布并生成 client/schema/MSW。**
...
| P12 | `/audit`、`/:correlationId`、`/exports/:exportId` | searchAuditEvents、getEvidenceChain、getExportStatus、getExportDownload | createExport、cancelExport | 无（异步 job 轮询） | 审/管/资源级；脱敏 | occurredAt、retentionUntil、download expiresAt | 1.3.0 / BFF-FE-007 | 1.3.0（本地参考 provider；staging 待签署） | BFF TL | 2026-09-16 |
...
| P15 | `/settings/profile`、`/notifications`、`/security` | getProfile、getNotificationPrefs、getSecuritySettings、listSessions、listDevices | saveProfile、saveNotificationPrefs、revokeSession、revokeDevice、setupMfa、revokeMfaFactor | subscribeSessionRevocations | 已登录本人 | session expiresAt、lastActiveAt、trustedUntil | 1.2.0 | 1.2.0 | BFF TL（本地参考 provider；staging 待签署） | 2026-09-16 |
...
| P17 | `/settings/browser` | getPlatformCapabilities、listDownloads | 无（浏览器授权仅由用户手势调用 Browser API；BFF 不扩大本地权限） | – | Web 已登录；Desktop 隐藏入口 | download expiresAt | 1.2.0 | 1.2.0 | BFF TL（本地参考 provider；staging 待签署） | 2026-09-16 |
...
mock 版本说明：生成 TypeScript client、Zod、JSON Schema 与 MSW handlers 与 OpenAPI 同源；当前版本 `1.3.0`。C01/C10/C17 P15 已有本地 Rust 参考 provider，但仍需 staging 消费者/provider 签署后才能升级为 Integrated；P16 仍属 BFF-FE-011。

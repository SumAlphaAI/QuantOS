...
**截至该次复验的结论（已由 10.1.14 取代）**：F09 已按正式部署结果确认通过。TP01-A/TP01-B 的只读 upstream、副本可重建、remote 契约和自动化拒绝门禁已完成；当时真实受控 fork 尚未创建，故 TP01 顶层清单未勾选。F0 Gate 当时为 **5/7**。
...
1. 使用具备 `sumalphai/Vibe-Trading` 仓库权限的 GitHub 身份确认或创建受控 fork，再运行 `make tp01-vibe-bootstrap`。
...
| 核查项 | 结果 | 完整记录 |
|---|---|---|
| 真实 fork 身份 | **通过** | GitHub metadata 确认 `sumalphai/Vibe-Trading` 的 `fork=true`，parent/source 均为 `HKUDS/Vibe-Trading`；公开 HTTPS remote 可读取。创建时 `main` 与官方 `main` 同为 `d16725e7...`。 |
| v0.1.13 基线一致性 | **通过** | 已批准 tag `v0.1.13` 对应 peeled commit `c33133f4fd5e978d21d2a61fdd8787fb352b4687`。只读 submodule、`baseline.lock.json`、`repository.lock.json`、`UPSTREAM.md` 与 SPDX SBOM 均统一至该 tag/SHA；LICENSE、NOTICE、requirements lock、pyproject 四项 SHA-256 与锁文件一致。 |

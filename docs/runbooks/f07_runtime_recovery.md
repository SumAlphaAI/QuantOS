...
6. Run `make f07-target-service-acceptance` only against the confirmed isolated project with `QUANTOS_F07_ISOLATED_PROJECT=1` and `QUANTOS_F07_ALLOW_TEMP_ADMIN_STORAGE_KEY=1`. This starts local BFF and Runtime binaries with independent narrow database logins, creates a temporary Supabase Auth owner, executes the real Runtime worker and Storage round trip, and writes `artifacts/f07/target-service.json`. The temporary Storage key is process-only; the test identity and append-only audit records remain in the isolated project for traceability. The receipt must report the exact clean source SHA before it can count toward acceptance.

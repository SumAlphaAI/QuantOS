...
1. Apply migrations through `20260925121000_f07_artifact_bucket.sql` to an isolated database, including the F07 Artifact binding, atomic scheduling, actor identity, and private Storage bucket migrations. Check `quantos_runtime` is a no-login role and the application login can set **only** that role. Do not use the operator, BFF or Execution login for the Runtime process.
...
6. Run `make f07-target-service-acceptance` only against the confirmed isolated project with `QUANTOS_F07_ISOLATED_PROJECT=1` and `QUANTOS_F07_ALLOW_TEMP_ADMIN_STORAGE_KEY=1`. This starts local BFF and Runtime binaries with independent narrow database logins, creates a temporary Supabase Auth owner, executes the real Runtime worker and Storage round trip, and writes `artifacts/f07/target-service.json`. The temporary Storage key is process-only; the test identity and append-only audit records remain in the isolated project for traceability. This is development-stage functional evidence only; the receipt remains `DIAGNOSTIC_ONLY` even with an exact clean source SHA.
...
F07 development acceptance requires same-source-SHA CI and Nightly results for database/RLS, 100 OS-kill recoveries without duplicate Artifact, cancel/timeout audit, schedule P95, and coverage; it also requires the F06 dependency Gate and a local BFF/Runtime service smoke test against isolated Supabase. A temporary administrator Storage key can be used only for that isolated functional diagnostic and never proves least-privilege deployment. See the [development acceptance record](../audit/F07-development-acceptance-2026-09-25.md).

Before publishing Runtime, L04 separately requires a deployed HTTPS BFF/Runtime route and a Runtime Storage credential scoped to `quantos-artifacts`, including denied access to other buckets, rotation/revocation, recovery, and a receipt tied to the release candidate SHA. Local acceptance does not waive those release checks.

"""Deterministic QuantOS LLMQuant signal service."""
...
from llmquant.adapter import LlmQuantRequestAdapter, RequestBoundaryError
from llmquant.fixtures import load_fixture
from llmquant.manifest import build_manifest
from llmquant.signal_mapper import map_signal


@dataclass
class LlmQuantService:
    """Contract-complete TP03 engine for deterministic QuantOS Signals."""

    manifest: EngineManifest = field(default_factory=build_manifest)
    request_adapter: LlmQuantRequestAdapter = field(default_factory=LlmQuantRequestAdapter)
    ...
    def execute(
        ...
        signal_artifact = common_pb2.ArtifactRef(
            artifact_id=f"signal:{request.workflow_run_id}",
            uri=(
                f"supabase://quantos-artifacts/tenant/{translated.context.tenant_id}/"
                f"llmquant/{request.workflow_run_id}/signal.json"
            ),
            media_type="application/json",
            sha256=mapped.signal_hash,
            classification=common_pb2.DataClassification.DATA_CLASSIFICATION_INTERNAL,
        )
        model_artifact = common_pb2.ArtifactRef(
            artifact_id=f"model-diagnostics:{request.workflow_run_id}",
            uri=(
                f"supabase://quantos-artifacts/tenant/{translated.context.tenant_id}/"
                f"llmquant/{request.workflow_run_id}/model-diagnostics.json"
            ),
            media_type="application/json",
            sha256=mapped.model_artifact["artifact_hash"],
            classification=common_pb2.DataClassification.DATA_CLASSIFICATION_INTERNAL,
        )
        return engine_pb2.ExecuteResponse(
            ...
            artifact_refs=[signal_artifact, model_artifact],
            evidence_refs=list(mapped.signal.evidence_refs),
            output=json_document_from_mapping(mapped.signal_payload),
    ...
    def stream_execute(
        ...
        signal_artifact = common_pb2.ArtifactRef(
            artifact_id=f"signal:{request.request.workflow_run_id}",
            uri=(
                f"supabase://quantos-artifacts/tenant/{translated.context.tenant_id}/"
                f"llmquant/{request.request.workflow_run_id}/signal.json"
            ),
            media_type="application/json",
            sha256=mapped.signal_hash,
            classification=common_pb2.DataClassification.DATA_CLASSIFICATION_INTERNAL,
        ...
        model_artifact = common_pb2.ArtifactRef(
            artifact_id=f"model-diagnostics:{request.request.workflow_run_id}",
            uri=(
                f"supabase://quantos-artifacts/tenant/{translated.context.tenant_id}/"
                f"llmquant/{request.request.workflow_run_id}/model-diagnostics.json"
            ),
            media_type="application/json",
            sha256=mapped.model_artifact["artifact_hash"],
            classification=common_pb2.DataClassification.DATA_CLASSIFICATION_INTERNAL,
        ...
        for index, event in enumerate(mapped.stream_events, start=1):
            ...
            yield engine_pb2.StreamExecuteResponse(
                ...
                artifact_refs=[signal_artifact, model_artifact] if is_last else [],
                done=is_last,
    ...
    def _prepare_execution(self, request: engine_pb2.ExecuteRequest, context: grpc.ServicerContext):
        try:
            ...
            mapped = map_signal(request.metadata, translated.context, fixture)

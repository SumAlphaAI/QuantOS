...
-- An execution grant cannot be checked without the account whose mode and
-- membership will be verified. A global grant may still apply to a specific
-- account after that account has passed the same-tenant mode check.
...
as $$
  select target_tenant_id is not null
     ...
     and (required_capability <> 'execution.operate' or target_account_id is not null)
     and exists (
       select 1

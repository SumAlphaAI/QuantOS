...
test.describe("UI-104 profile, security, notifications and browser settings", () => {
  ...
  test("P15 security and P17 browser 1440 dark visual baselines", async ({ page }) => {
    await page.goto("/settings/security");
    await expect(page).toHaveScreenshot("ui104-security-1440-dark.png", { fullPage: true, maxDiffPixelRatio: 0.005 });
    await page.goto("/settings/browser");
    await expect(page).toHaveScreenshot("ui104-browser-1440-dark.png", { fullPage: true, maxDiffPixelRatio: 0.005 });
  });
});
